...
Kao što je već navedeno Dionyzus je 3D igra, što znači da su sav okoliš, glavni lik
...
%, animacije su besplatne te je preuzimanje istih vrlo jednostavno. Sve što je
...
Sve najvažnije komponente koje su potrebne kako bi lik pravilno funkcionirao nalaze
...
Osim omogućavanja kretnji potrebno je i zabilježiti iste te znati gdje se lik kreće.
...
Ono što je vidljivo u obje skripte je ~\texttt{PlayerState}. To je skripta koja sadrži sva
...
Sva logika se odvija unutar ~\texttt{Update} metode gdje se konstantno osluškuje unos
...
Sistem oružja je izveden preko nekoliko skripti koje su prikazane u
...
~\texttt{Shooter} omogućava pucanje. Dodavanjem ove skripte na objekt, potrebno je postaviti
...
Sljedeća skripta je ~\texttt{WeaponController} u kojoj je opisana logika korištenja oružja
...
~\texttt{WeaponReloader} je jednostavna skripta koja omogućava ponovno punjenje oružja
...
\end{lstlisting}
Ključna riječ je ~\texttt{RequireComponent}, prima tip komponente koja će se automatski
...
Unutar ove skripte, ali i unutar kompletnog projekta često je vidljiv atribut
...
~\texttt{EnemyHealth} i ~\texttt{EnemyAnimation} su implementiran na identičan način kao već opisane
skripte za iste funkcionalnosti igrača te zbog toga nisu detaljnije prikazane. 

~\texttt{EnemyShoot} skripta implementira ponašanje neprijatelja u trenutku kad detektira
igrača. Nakon što neprijatelj otkrije igrača prestaje s patroliranjem te ulazi u
stanje svjesnosti te napada igrača, počinje ciljati i pucati. To je riješeno tako da
puca u nasumičnim intervalima i u kratkim rafalima. Iako vrlo precizno prema igraču,
ipak uz mali pomak kako bi pucanje bilo što realnije. Osim toga neprijatelj se
saginje te ponovno puni oružje ako potroši sav spremnik. U slučaju da igrač nije
više u vidnom polju neprijatelja, neprijatelj mijenja stanje u nesvjesno te nakon
kraćeg perioda nastavlja s patroliranjem. K\^od skripte nalazi se u
ispisu~\ref{EnemyShoot}.
...
ActiveWeapon.AimTarget.position);
    ...
    void StartBurst()
    {
        ...
        GameManager.Instance.Timer.Add(EndBurst, Random.Range(burstDurationMin,
burstDurationMax));
    }
    void EndBurst()
    {
...
Prikupljanje stvari iz prostora je riješeno pomoću ~\texttt{PickupItem} skritpe. Unutar
...
Ono što je implementirano unutar Dionyzus projekta je ulazak i izlazak iz automobila
...
Osim osnovnih elemenata, tijekom igranja igraču se prikazuju tzv.~uputstva, odnosno
